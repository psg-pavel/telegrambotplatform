...
$tab = mysqli_query($conn, "SELECT * FROM access WHERE `role`='2'");?>
...
<?while ($result = mysqli_fetch_array($tab)) { 
      ...
      echo "<tr style='height:25px !important;'>
            ...
            <td>
            <div>"; if ($login_login !== 'adm') {echo $login_pass;}else{echo '****';} echo "</div>
            </td>
            <td style='text-align:center; padding-top:10px;'>
            <div >
                  <form method='POST'>
                        <button name='submit' type='submit' style='width: 65px;'>удалить</button>
                        <input name='adm_del' type='hidden' value='".$login_login."'>
                        <input name='adm_name_del' type='hidden' value='".$login_name."'>
                  </form>
            </div>
            </td></tr>";}?>
            ...
            <form method='POST'>
                  <input id='times' name='adm_log_add' type='text' value='логин'>
                  <input id='times' name='adm_name_add' type='text' value='имя'>
                  <input id='times' name='adm_pass_add' type='text' value='пароль'>
